...
def temp_converter(inp):
    return (inp-32)*5/9

def speed_converter(inp):
    return inp*1.60934

def time_converter(inp):
    """
    inp: 11/07/2022, 04:09:13
    """
    hour = int(inp.split(',')[1].split(':')[0])+8
    date = inp.split(',')[0]
    mm,ss = inp.split(',')[1].split(':')[1],inp.split(',')[1].split(':')[2]
...
def create_df():
    fdf = pd.read_csv('/root/pushGithub/forecast_pre.csv')
    hdf = pd.read_csv('/root/pushGithub/history_pre.csv')
...
def create_aqi_df():
    afdf = pd.read_csv('/root/pushGithub/aqi_forecast_pre.csv')
    ahdf = pd.read_csv('/root/pushGithub/aqi_history_pre.csv')
...
def get_n_most_recent_forecast_series(df_forecast, column_main, window, window_unit):
    ...
    now = datetime.datetime.now(pytz.timezone('Asia/Kuala_Lumpur')).replace(tzinfo=None)
    # print(now)
    # print(type(now),type(df_forecast["datetime"]))
    # print(df_forecast["datetime"])
    timedeltas = now - pd.to_datetime(df_forecast["datetime"])
    ...
    forecast2show_fetchstrs = df_forecast2show["fetched_at"].unique()
...
def create_time_figure(
    ...
    if show_forecast:
        ...
        for i, fetched_time in enumerate(forecast2show_fetchstrs):
            df_fetched_at = df_forecast[df_forecast["fetched_at"] == fetched_time]
    ...
    if as_type == "figure":
        ...
        if limit_date_range:
            ...
            if datetime.datetime.now() - pd.to_datetime(df_history["datetime"]).min() > limited_td:
...
def wind_direction_graph(df_wind, history_rgb, forecast_rgb):
    ...
    timedeltas = datetime.datetime.now() - pd.to_datetime(df_history["datetime"])
    ...
    history_polar = go.Scatterpolar(
        ...
        customdata=df_history["fetched_at"],
    ...
    most_recent = go.Scatterpolar(
        ...
        customdata=[df_history["fetched_at"].loc[ix_most_recent]],
        hovertemplate='time:%{customdata} <br>speed:%{r:.3f}<br>direction:%{theta}'
    )
    ...
    for i, fetched_time in enumerate(forecast2show_fetchstrs):
        ...
        forecast_graph = go.Scatterpolar(
            ...
            visible="legendonly",
            customdata=df_fetched_at["datetime"],
            hovertemplate=hovertemplate
        )
...
def generate_page(n_intervals):
    try:
        ...
        df['temperature.temp'] = [temp_converter(i) for i in df['temperature.temp'].tolist()]
        df['temperature.temp_min'] = [temp_converter(i) for i in df['temperature.temp_min'].tolist()]
        df['temperature.temp_max'] = [temp_converter(i) for i in df['temperature.temp_max'].tolist()]
        df['temperature.feels_like'] = [temp_converter(i) for i in df['temperature.feels_like'].tolist()]
        df['wind.speed'] = [speed_converter(i) for i in df['wind.speed'].tolist()]
        ...
        df["timedeltas_datetime"] = pd.to_datetime(df["datetime"]) - datetime.datetime.now()
        df["timedeltas_fetched"] = datetime.datetime.now() - pd.to_datetime(df["fetched_at"])
        ...
        most_recent_forecast_fetched_at = df["fetched_at"].loc[most_recent_forecast_idx]

        df_forecast_most_recent = df[
            (df["origin"] == "forecast") & (df["fetched_at"] == most_recent_forecast_fetched_at)]
        ...
        df_hm["dow"] = pd.to_datetime(df_hm["datetime"]).apply(lambda x: DOWS[x.weekday()])
        # print(df_hm['datetime'])
        # print(df_hm['dow'])
        df_hm["hour"] = pd.to_datetime(df_hm["datetime"]).apply(lambda x: x.hour)

        square_index = list(reversed(df_hm["dow"].unique().tolist()))
        hours_index = [2,5,8,11,14,17,20,23]

        df_square_statuses = pd.DataFrame("Clouds", columns=hours_index, index=square_index)
        ...
        for entry in df_hm.iterrows():
            ...
            # df_square_ints.at[d, h] = "Clouds" if s == "no status" else s
        ...
        # print(df_square_ints)
        # x_value = [(i+8)%24 for i in df_square_ints.columns.tolist()]
        # df_square_ints.column = x_value
        ...
        figh.update_layout(
            ...
            yaxis=dict(title="Day", showgrid=False)
        ...
        right_now = html.Div([
            ...
            html.Table([
                html.Tr([
                    ...
                    html.Td(f'{int(most_recent_weather["temperature.temp"])}C (feels like {int(most_recent_weather["temperature.feels_like"])}C)', style=white_text)
                ]),
                html.Tr([
                ...
                html.Tr([
                    ...
                    html.Td(f'{speed_converter(wind_speed)} kph @ {wind_cardinal}', style=white_text)
                ]),
                html.Tr([
                    html.Td("Fetched at", style=white_text),
                    html.Td(f'{most_recent_weather["fetched_at"]}', style=white_text)
                ]),
                html.Tr([
                    html.Td("Potential Flood Warning", style=white_text),
                    html.Td("None",style=red_text)
                ])
            ])
        ...
        return html.Div([toprow] + [html.H3("Recent Weather Metrics", style=white_text)] + divs )
            ...
            children=[
                html.Div(
                    children=[
                        ...
                        html.H1(children='Floody: Weather dashboard for Kuala Lumpur', style={"color": "white"}),
                        html.Div(children='''
                            Real-time data update every three minutes. Prediction updates every three hours.
                        ''', style={"color": "white"}),
                        html.Br(),
                        html.Br(),
                    ],
...
if __name__ == '__main__':
    # main()
    # app.run_server(debug=False, host="0.0.0.0", port=6789)
